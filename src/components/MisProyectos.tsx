            ...
            proyectos.map((p) => {
              ...
              return (
                <div key={p.id}
                  ...
                  style={{ borderColor: "rgba(255,255,255,0.05)", background: isActual ? "rgba(124,58,237,0.06)" : "transparent" }}
                  onClick={() => onAbrir(p)}>

                  {/* Icono */}
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
                    style={{ background: isActual ? "rgba(124,58,237,0.12)" : "rgba(255,255,255,0.04)", border: `1px solid ${isActual ? "rgba(124,58,237,0.3)" : "rgba(255,255,255,0.06)"}` }}>
                    <FolderOpen size={14} style={{ color: isActual ? "#a78bfa" : "#52525b" }} />
                  </div>

                  {/* Info */}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold truncate" style={{ color: isActual ? "#a78bfa" : "#e4e4e7" }}>
                        {p.nombre}
                      </span>
                      {isActual && (
