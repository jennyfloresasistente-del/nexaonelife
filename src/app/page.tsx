...
export default function NexaOnePage() {
  ...
  const { user, isLoggedIn, login, register, logout } = useAuthStore();
  ...
  const handleAuth = useCallback(async () => {
    ...
    const result = authMode === "login"
      ...
      : register(authEmail, authPassword, authNombre);
  ...
  }, [authMode, authEmail, authPassword, authNombre, login, register]);
      ...
      <header className="flex items-center justify-between px-4 py-2.5 shrink-0"
        ...
        <div className="flex items-center gap-1.5">
          ...
          {isLoggedIn && user ? (
            <div className="flex items-center gap-1.5">
              <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold"
                ...
                <div className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-black text-white"
                  style={{ background: "linear-gradient(135deg,#7c3aed,#06b6d4)", fontSize: "9px" }}>
                  {user.avatar}
                </div>
                <span className="hidden sm:inline max-w-[80px] truncate">{user.nombre}</span>
              </div>
            ...
            <button
              onClick={() => { setShowLoginModal(true); setAuthMode("login"); setAuthError(""); }}
